...
// These are unchanged and can be copied from the previous response.
...
export function StatsSection() {
  ...
  // The horizontal radius is now larger than the vertical one.
  ...
  const stats = [
    ...
    { value: mockStats.response, label: "Emergency Response", description: "Average response time in minutes", icon: Zap },
  ].map((stat, index) => {
    ...
    >
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="w-full">
          ...
          <div className="hidden lg:flex items-center justify-center relative h-[680px]"> {/* Adjusted height for horizontal layout */}
            ...
            {stats.map((stat, index) => {
              ...
              return (
                <div
                  key={index}
                  className={`group absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 transition-opacity duration-700 ${isInView ? 'opacity-100' : 'opacity-0'}`}
                  style={{ ...stat.style, animationDelay: `${index * 0.15}s` }}
                >
                  <div className="w-47 h-47 rounded-full bg-slate-800/50 backdrop-blur-sm border border-blue-500/20 shadow-2xl shadow-blue-900/50 transition-all duration-300 group-hover:scale-110 group-hover:shadow-blue-600/60 flex flex-col items-center justify-center text-center p-2">
                    ...
                    <div className="absolute bottom-full mb-2 w-max px-3 py-1.5 text-sm font-medium text-white bg-slate-900/80 rounded-lg shadow-sm opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none">{stat.description}</div>
                  </div>
                </div>
              );
            })}
